        ...
        try:
            ...
            if message:
                if message.decode('utf-8')[:5] == "check":
                    ...
                    send_to_client(client_socket, f"Current Word: {''.join(player_word)}")
                    logging.info(f"Client {client_id} checked their word: {''.join(player_word)}")
                    ...
                    send_to_client(client_socket, f"Updated Word: {updated_word}")
                    
                    # Optionally log the updated word
                    logging.info(f"[{client_id}] Guessed {guessed_letter}, Updated word: {updated_word}")
